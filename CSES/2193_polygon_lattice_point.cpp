...
struct line
{
    ...
    T relationship(const P &p) const { return (p1 - p).dot(p2 - p); }           // > 0, out of segment; = 0, on end; < 0 inside (Projection!)
    bool on_seg(const P &p) const { return !relationship_normal(p) && relationship(p) <= 0; }
...
struct polygon
{
    ...
    int relationship(const P &p) const // 1, outside; 0, on edge; -1, inside
    {
        int y = 1;
        for (int i = 0; i < n; i++)
            if (L{points[i], points[(i + 1) % n]}.on_seg(p))
                return 0;
            else if ((points[i].y > p.y) ^ (points[(i + 1) % n].y > p.y) && p.x < 1.l * (p.y - points[i].y) * (points[(i + 1) % n].x - points[i].x) / (points[(i + 1) % n].y - points[i].y) + points[i].x)
                y = -y;
        return y;
    }
};
...
int main()
{
    ...
    for (auto &[x, y] : pg.points)
        cin >> x >> y;
    int64_t b = 0; // # lattice points on boundary
    for (int i = 0; i < n; i++)
        b += __gcd(abs(pg.points[i].x - pg.points[(i + 1) % n].x), abs(pg.points[i].y - pg.points[(i + 1) % n].y));
    cout << (pg.area2() - b + 2 >> 1) << ' ' << b << '\n';
    return 0;
}