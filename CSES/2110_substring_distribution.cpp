...
vector<int64_t> suffix_array(const string &s)
{
    int n = s.length(), m = 256;
    vector<int64_t> y(n), bucket(max(n + 1, m)), rolling[2];
...
vector<int64_t> lcp_array(const string &s, const vector<int64_t> &sa)
{
    int n = s.length();
    vector<int64_t> y(n), rk(n);
    for (int i = 0; i < n; i++)
        rk[sa[i]] = i;
...
int main()
{
    ...
    partial_sum(ALL(v), ostream_iterator<int64_t>(cout, " "));
    cout << '\n';
    return 0;
}