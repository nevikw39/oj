...
constexpr int64_t Q = 5e5 + 5;
vector<pair<int, int>> time_segment_tree[Q << 2];
vector<int> y;
...
void solve()
{
    ...
    for (int i = 0; i < q; i++)
    {
        ...
        if (a > b)
            swap(a, b);
        if (c == 'N')
            hm[a * Q + b].push_back(i);
        else
        {
            insert(hm[a * Q + b].back(), i, {a, b}, 1, 0, q);
            hm[a * Q + b].pop_back();
        }
    }
    for (auto [x, v] : hm)
